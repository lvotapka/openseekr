...
atomic_weights = {'H':1.00800, 'C':12.0110, 'N':14.00699, 'O':15.9994, 
                  ...
                  'Fe':55.845, 'Br':79.904, 'I':126.9045, '':0.0, ' ':0.0, 
                  'G':0.0} # all the atomic weights. NOTE: 'G' refers to a 
                           # "ghost" atom
radii = {'C': 1.9080, 'N':1.824, 'H':0.0000, 'O':1.66, 'S':2.000, 'Ca':1.97, 
         'P':2.00, 'Cu':2.5, 'Zn':2.5, '':0.0, "G":0.0}
TER_atoms = ['OXT', 'Ca+', 'CL'] # atom names after which automatically add a TER card if amber mode activated
...
class Structure():
    def __init__(self, struct_id):
        ...
        self.num_atoms = 0
        self.num_resids = 0
        self.resids = [] # a list of lists of atoms
        self.sequence = ""
        self.ter_list = []
        return
    ...
    def get_sequence(self):
        '''Reads this structure's atoms and residues to construct an amino acid sequence'''
        oldresid = ""
        seq_list = []
        resid_list = []
        resid_sublist = []
        for atom in self.atoms: # loop thru the atoms
            if atom.resid != oldresid: # we've hit the beginning of a new residue, so append to the list
...
pqrregex = re.compile(r'^([A-Z]+) *([0-9a-f\*]+) *([\S]{1,4}) *([\S]{3,4}| [\S]{2}|  [\S]) +([@A-Z]{0,1}) *([-0-9a-f]+)([a-zA-Z]{0,1}) *([- ]{0,1}[0-9]+\.[0-9]{3}) *([- ]{0,1}[0-9]+\.[0-9]{3}) *([- ]{0,1}[0-9]+\.[0-9]{3}) *([- ]{0,1}[0-9]+\.[0-9]{1,4}) *([- ]{0,1}[0-9]+\.[0-9]{1,4}) *(\S{0,2}) *$')

def get_AA_code(amino3code, unknown='X'):
    '''Given the 3-letter amino acid code, will return a letter representing that amino acid. Unknown values are filled with the 'unknown' value.'''
    if amino3code in list(protein_dict.keys()):
        return protein_dict[amino3code]
    else:
        return unknown

class Big_PDBParser():
    def pdbparse(self, filename, preserve_index = False, 
                 preserve_resid = True, pqr=False, conventional=False, 
                 multi_frame=False):
        ...
        if multi_frame == True:
            pdb_struct_list = []
        if isinstance(filename, io.IOBase): # then we're passing a file or file-like object
            pdbfile = filename
        ...
        for pdbline in pdbfile:
            if pdbline.startswith('END'):
                if multi_frame == True:
                    ...
                    atoms = []; ter_list = []; counter = 1; residcounter = 0; oldresid = 0 # reset everything
                else:
                    pdbfile.close()
                    break
            if pdbline.startswith('TER'):
                ter_list.append(residcounter)
                #if multi_frame == True: # TODO: I don't like this cookie-cutter code, find a way to combine with "END" above
                #  pdb_struct_list.append(atoms)
                #  atoms = []; ter_list = []; counter = 1; residcounter = 0; oldresid = 0 # reset everything
            ...
            counter += 1
            #if type(filename) == str: pdbfile = open(filename, 'r') # TODO: marked for removal
    ...
    def pdbparseCONECT(self, filename, remove_redundant_bonds=True, 
                       multi_frame=False):
        '''
        Parse the CONECT section of the PDB file in order to find connected 
        atoms
        '''
        bonds = []
        bonds_set_list = []
        visited_indices = [] # atoms we've already made a connection to
        if isinstance(filename, io.IOBase): # then we're passing a file or file-like object
            pdbfile = filename
        elif type(filename) == str: # otherwise, try to open it as a string
            pdbfile = open(filename, 'r')
        ...
        with pdbfile:
            for pdbline in pdbfile:
                ...
                elif pdbline.startswith('CONECT'):
                    bonded_atom_indices = []
                    atom_index = int(pdbline[7:12].strip())-1
                    bonded_atom_indices.append(int(pdbline[12:17].strip())-1)
                    bonded_atom_indices.append(int(pdbline[17:22].strip())-1)
                    bonded_atom_indices.append(int(pdbline[22:27].strip())-1)
                    bonded_atom_indices.append(int(pdbline[27:31].strip())-1)
                    if atom_index not in visited_indices:
                        visited_indices.append(atom_index)
                    for bonded_atom_index in bonded_atom_indices:
                        if bonded_atom_index == -1: # not actually a bond
                            continue
                        if remove_redundant_bonds:
                            if bonded_atom_index in visited_indices:
                                continue # this bond already accounted for
                        bonds.append([atom_index, bonded_atom_index])
        
        
        if multi_frame:
            return bonds_set_list
        else:
            return bonds
    ...
    def conventional_parse(self, line, pqr=False): # an emergency function that attempts to parse the line of a pdb file according to conventional rules
        ...
        stripped_line_list = []
        for line in rawlinelist[0]:
            stripped_line_list.append(line.strip())
    ...
    def get_structure(self, struct_id, filename, preserve_index=False, 
        ...
        if pqrxml:
            pqrxml_generator = pqr_xml_parse(
                filename, preserve_resid=preserve_resid, 
            ...
            for atom_dict,num_res in pqrxml_generator:
                ...
                atom = Atom(record="ATOM", index=atom_dict['index'], 
                            ...
                            radius=atom_dict['radius'], occupancy='1.0', 
                            beta='0.0', )
                structure.atoms.append(atom)
            structure.num_resids = num_res
            return structure
        else:
            #structure.atoms, structure.num_resids, structure.sequence, structure.resids = self.pdbparse(filename,preserve_index,preserve_resid,pqr=pqr,conventional=conventional)
            if multi_frame == True:
                structure_list = []
                atoms_list, num_resids, ter_list = self.pdbparse(filename,preserve_index,preserve_resid,pqr=pqr,conventional=conventional,multi_frame=True)
                print("ter_list:", ter_list)
                print("len(atoms_list):", len(atoms_list))
                print("num_resids:", num_resids)
                for atoms in atoms_list:
                    structure = Structure(struct_id)
                    structure.atoms, structure.num_resids, structure.ter_list = atoms, num_resids, ter_list
                    structure.num_atoms = len(structure.atoms)
                    structure_list.append(structure)
                return structure_list
            else:
                structure.atoms, structure.num_resids, structure.ter_list = self.pdbparse(filename,preserve_index,preserve_resid,pqr=pqr,conventional=conventional,multi_frame=False)
                structure.num_atoms = len(structure.atoms)
            #NOTE: There is no way so far to assign structure.num_resids
            structure.bonds = self.pdbparseCONECT(filename, 
                                                  remove_redundant_bonds=True, 
...
def rmsd(structure1, structure2):
    '''returns the rmsd between two structures with the same number of atoms.'''
    coords1 = getcoords(structure1)
    coords2 = getcoords(structure2)
    assert len(coords1) == len(coords2), "RMSD can only be found between structures with the same number of atoms."
...
def load_pdb_traj(infilename):
    ...
    traj =[]
    starttime = time.time()
    pdbtemp = io.StringIO() # a file-like object to hold the current pdb file to be opened later by the pdb parser
