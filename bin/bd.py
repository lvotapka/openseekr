...
def write_browndye_input(pqrs,seekrcalc,criteria,work_dir='.',browndye_bin='', start_at_site='true',fhpd_mode=False):
    '''
    ...
    #inputgen_settings = seekrcalc.browndye.apbs.inputgen #settings['inputgen_settings']
    ...
    for pqr in pqrs: # for each molecule in pqr format
        ...
        dxfile, debye = apbs.main(seekrcalc, pqrfile, fhpd_mode=fhpd_mode) # get the electrostatic grid and debye length for the molecule
        debyes.append(debye)
    ...
    input_xml['root']['solvent']['debye-length'] = str(max(list(map(int,list(map(float,debyes)))))) # set the debye length
    input_xml['root']['reactions'] = RXN_FILENAME
    input_xml['root']['solvent']['kT'] = seekrcalc.master_temperature #float(settings['temperature']) / DEFAULT_TEMP
...
if __name__=='__main__':
    if __name__ == "__main__" and 'INPUTGEN' not in os.environ:
        print("In order to run unit tests in bd.py, please set the BROWNDYE_BIN environmental variable to the directory containing the BrownDye programs")
        exit()
