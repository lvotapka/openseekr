...
This is a module to test the SEEKR package and to aid in its development
and introduction of functionality
"""
import sys
import numpy as np
...
# create a new SEEKR calculation object
me = seekr.SeekrCalculation()
#*kelvin # temperature of (most) all calculations
me.master_temperature = 298. 
...
# Make sure this is selected by "index" in VMD
rec_site_atom_indices = [2478, 2489, 2499, 2535, 2718, 2745, 2769, 2787, 
                         2794, 2867, 2926] 
...
me.building.lig_dry_pqr_filename = \
  "/home/lvotapka/tryp_files/benzamidine_moved.pqr"
me.building.rec_wet_pdb_filename = \
  "/home/lvotapka/torq/lvotapka/Documents/trypsin_files/apo_redo/\
      tryp_wet_1946.pdb"
me.building.rec_dry_pqr_filename = \
  "/home/lvotapka/torq/lvotapka/Documents/trypsin_files/apo_redo/\
      tryp_dry_1946.pqr"
...
#[PDBReporter('dummy', 500)] # SEEKR will automatically 
# change the filename
me.min_equil.min_reporter_freq = 500 
me.min_equil.temp_equil_integrator = LangevinIntegrator(
    me.master_temperature*kelvin, 5/picosecond, 0.002*picoseconds)
# SEEKR will automatically change the filename
me.min_equil.temp_equil_reporters = [PDBReporter("dummy", 100)] 
# number of simulation steps per temperature
me.min_equil.temp_equil_steps = 1000 
# progression of the temperature equilibration
me.min_equil.temp_equil_temperatures = [300., 310., 320., 330., 340., 350., 
                                        340., 330., 320., 310., 300.] 

# BrownDye information
me.browndye.browndye_bin_dir = "/home/lvotapka/Downloads/browndye/bin"
me.browndye.num_threads = 10
me.browndye.lig_dry_pqr_filename = \
    "/home/lvotapka/tryp_files/benzamidine_moved.pqr"
me.browndye.rec_dry_pqr_filename = \
    "/home/lvotapka/tryp_files/tryp_dry.pqr"
me.browndye.prods_per_anchor = 1000000
me.browndye.apbs.executable = \
    "/home/lvotapka/Downloads/APBS-1.5-linux64/bin/apbs"
me.browndye.fhpd_numtraj = 1000

ion1 = seekr.APBS_ion("Cl-", 0.10, -1.0, 1.67)
# define ions in the system
ion2 = seekr.APBS_ion("Ca2+", 0.02, 2.0, 1.14) 
ion3 = seekr.APBS_ion("tris", 0.06, 1.0, 4.0)


me.browndye.apbs.ions = [ion1, ion2, ion3]
me.browndye.apbs.linear_pbe = False
me.browndye.apbs.inputgen.executable = \
  "/home/lvotapka/Downloads/APBS-1.5-linux64/share/apbs/tools/\
      manip/inputgen.py"
...
milestones = seekr.generate_spherical_milestones(
    me, rec_site_atom_indices, origin, radius_list, 
    0, vectors, absolute=False)
...
holo_config_wet, insert_index, last_ligand_index = \
  seekr.generate_configs(me)
ligand_heavy_indices = seekr.find_heavy_atoms(
    holo_config_wet, 
    range(insert_index, insert_index+last_ligand_index+1))
# ATTN: changed
...
loadoff /home/lvotapka/torq/lvotapka/Documents/trypsin_files/\
    benzamidine.lib
loadamberparams /home/lvotapka/torq/lvotapka/Documents/trypsin_files/\
    benzamidine.frcmod
WAT = T4E
HOH = T4E
...
for milestone in me.milestones:
  ...
  if milestone.md:
    ...
    parm = pmd.load_file(milestone.openmm.prmtop_filename, 
                         xyz=milestone.openmm.inpcrd_filename)
    
    #TODO: straighten out these units
    parm.box = np.array([64.9127105, 64.9127105, 64.9127105,109.471219, 
                         109.471219,109.471219])
    box_vector = Quantity(
        [[64.912710500000003, 0.0, 0.0], 
        [-21.637568420791037, 61.200290990259163, 0.0], 
        [-21.637568420791037, -30.600141791568205, 53.00100885481632]], 
        unit=angstrom)
    ...
    milestone.openmm.simulation.context.setPeriodicBoxVectors(
        [6.4912710500000003, 0.0, 0.0], 
        [-2.1637568420791037, 6.1200290990259163, 0.0], 
        [-2.1637568420791037, -3.0600141791568205, 5.300100885481632])
        
    # create a sample of a simulation file for future use
    if not me.openmm.simulation: 
