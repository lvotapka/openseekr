...
def make_apbs_input_using_inputgen(inputgen_filename, pqr_filename, fadd=60, cfac=4.0, gmemceil=64000, resolution=0.5, ionic_str=0.15):
    ...
    if verbose: print("Now creating APBS input file using command:", runstring)
    os.system(runstring)
    os.chdir(olddir)
...
def run_apbs (apbs_filename, input_filename, pqr_filename, std_out="apbs.out"):
    ...
    if verbose: print("copying file: %s to directory: %s" % (pqr_filename, os.path.join(rundir,os.path.basename(pqr_filename))))
    ...
    if os.path.exists(dx_filename):
        if verbose: print("DX file found:", dx_filename)
        if verbose: print("skipping APBS run...")
    else:
        os.chdir(rundir) # we want to run APBS in the directory
        if verbose: print("running command:", runstring)
...
def get_debye_length(apbs_std_outfilename, seekrcalc=None):
    ...
    apbs_std_outfile = open(apbs_std_outfilename, 'r')
    for line in apbs_std_outfile:
        m = re.search(debye_string, line)
        if m: # then we've found a line
            number_obj = re.search("[0-9.]+", line).group()
            if number_obj == "0":
                print("ALERT: Debye length of zero found. This may mean that your PQR file has a net charge that is NOT zero, or that your ion concentration was zero...")
            else:
                debye_list.append(number_obj)
    if len(debye_list) < 1:
        print("APBS was unable to find a Debye length. Now computing using a formula...")
        denominator = 0.0
        for ion in seekrcalc.browndye.apbs.ions:
            denominator += ion.concentration * molecules_per_m3_per_moles_per_liter * (ion.charge*coulombs_per_proton)**2
        inside = float(permittivity * k_B * seekrcalc.master_temperature / denominator)
...
def flatten_ion_list(apbs_settings):
    if 'ions' not in list(apbs_settings.keys()): return apbs_settings
...
def main(seekrcalc, pqr_filename,fhpd_mode=False):
    ...
    if not os.path.exists(inputgen_location):
        print("provided inputgen.py location does not exist. Please modify browndye.inputgen.executable value.")
        exit()
    input_filename = make_apbs_input_using_inputgen(inputgen_location, pqr_filename, fadd=inputgen.fadd, cfac=inputgen.cfac, gmemceil=inputgen.gmemceil, resolution=inputgen.resolution, ionic_str=inputgen.ionic_str)
    ...
    debye = get_debye_length(apbs_out, seekrcalc)
    return dx, debye
