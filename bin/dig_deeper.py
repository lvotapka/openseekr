...
def find_closest_ligand_orientation(prmtop, dcd_list, reference_top, 
                                    ...
                                    center_atom_indices):
    '''Reads in the dcd_list files one by one, extracting their final frames, 
    then compare them to a reference structure. The protein structures are 
    aligned, then the center of masses of the dcd last frame and the reference 
    are superimposed. Then the RMSD is measured. The frame with the smallest 
    RMSD is extracted.
    Input:
    ...
    assert len(ref_lig) > 0, \
        "no reference atoms selected for ligand with ligname %s" % ligname
    ...
    for dcd_file in dcd_list:
        ...
        last_frame_site = seekr.load_last_mdtraj_frame(
            dcd_file, prmtop, atom_indices=[x-1 for x in center_atom_indices])
        ...
        assert len(ref_lig) > 0, \
            "no last_frame atoms selected for ligand with ligname %s" % ligname
        assert len(ref_alpha_carbons) == len(last_frame_alpha_carbons), \
            'The reference and the dcd must have the same number of alpha '\
            'carbons'
        assert len(ref_lig) == len(last_frame_lig), \
            'The reference and the dcd must have the same number of ligand '\
            'atoms'
        ...
        dist_from_origin = np.sqrt(
            (lig_com[0, 0, 0]-site_com[0, 0, 0])**2 + \
        ...
        print("rmsd of frame %i" % counter, rmsd, 
              "distance from milestone origin:", dist_from_origin, "nm")
        if rmsd < best_rmsd:
            best_rmsd = rmsd
...
def read_data_file_transitions_down(data_file_name, destination='1', 
                                    last_frame=True):
    '''Read transition data file, return the first instance of a transition to 
    a lower milestone. Return the file index of the forward trajectory to this 
    lower milestone.
    Input:
     - data_file_name: string of the filename of a milestone's forward 
     transition file
     - destination: the string to search for. In the downward milestone case: 
        '1'. Upward would be '3'.
    Output:
    ...
    assert len(downward_indices) != 0, "FAILURE: no downward forward '\
        'trajectories detected. Please run additional umbrella sampling and '\
        'rev/fwd trajectories."
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        ...
        print("python dig_deeper.py MILESTONE PICKLE similar REF_PARM7 REF_RST7 "\
              " LIG_RESNAME")
        ...
        print("if the last argument is 'up' then an upward-going trajectory is "\
              "chosen.")
        exit()
    ...
    fwd_rev_dir = os.path.join(me.project.rootdir, milestone.directory, 'md', 
                               'fwd_rev')
    lower_temp_equil_dir = os.path.join(me.project.rootdir, 
                                        lower_milestone.directory, 'md', 
                                        'temp_equil')
    lower_temp_equil_filename = os.path.join(lower_temp_equil_dir, 
                                             'equilibrated.pdb')
    lower_milestone_holo = os.path.join(me.project.rootdir, 
                                        lower_milestone.directory, 'md', 
                                        'holo_wet.pdb')
    lower_milestone_building = os.path.join(me.project.rootdir, 
                                            lower_milestone.directory, 'md', 
                                            'building')
    ...
    pdb_list = sorted(glob.glob(os.path.join(fwd_rev_dir, 'forward*.pdb')), 
                      key=seekr.sort_forward_dcd_key)
    
    print("Writing new structures and files needed to run umbrella simulation on "\
          "the lower milestone (milestone %d)" % lower_milestone.index)
    ...
    if method=='first':
        ...
        downward_pdb = pdb_list[downward_indices[0]]
        print("Extracting frame from file:", downward_pdb)
        last_fwd_frame = seekr.load_last_mdtraj_frame(downward_pdb, prmtop)
    elif method=='last':
        #downward_fwd_dcd = os.path.join(fwd_rev_dir, 
        #                                'forward%i_0.dcd' % downward_indices[-1])
        #downward_dcd = dcd_list[downward_indices[-1]]
        downward_pdb = pdb_list[downward_indices[-1]]
        print("Extracting frame from file:", downward_pdb)
        last_fwd_frame = seekr.load_last_mdtraj_frame(downward_pdb, prmtop)
    elif method=='similar':
        #dcd_downward_list = []
        pdb_downward_list = []
        for pdb_index in downward_indices:
            #dcd_list.append(os.path.join(fwd_rev_dir, 
            #                'forward%i_0.dcd' % dcd_index))
            pdb_downward_list.append(pdb_list[pdb_index])
        last_fwd_frame = find_closest_ligand_orientation(
            prmtop, pdb_downward_list, ref_parm7, ref_rst7, lig_resname, 
            lower_milestone.center_atom_indices)
    elif method=='index':
