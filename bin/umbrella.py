...
def load_last_mdtraj_frame(dcd_filename, prmtop_filename, atom_indices=None):
  """
  This function returns the last frame of a DCD file as an MDtraj 
  object.
  Input:
   - dcd_filename: string that represents the path to the dcd file 
       to load
   - prmtop_filename: string that represents the path to the parm7 
       file to load
  Output:
   - lastframe: an mdtraj Trajectory object that contains a single 
       frame: the last one in the dcd.
  """
...
def create_forces(seekrcalc, milestone, system):
  """
  Add the umbrella force: which maintains the ligand on the surface 
  of the spherical milestone.
  Input:
  ...
  if verbose: print "k:", seekrcalc.umbrella_stage.force_constant, "radius:", \
      milestone.radius*angstrom, "g1:", milestone.atom_selection_1, "g2:", \
...
def launch_umbrella_stage(seekrcalc, milestone, box_vectors=None, 
  ...
  Input:
   ...
   - milestone: the Milestone() object to run the simulation for
  Output:
   - ending_box_vectors: An array of three vectors that represents 
        the ending state of the periodic box. This may not be the same 
        as was started, but could have changed through the course of a 
  ...
  if os.path.exists(pdb_filename):
    ...
    print "Restarting failed umbrella stage from last frame of DCD file: ", \
        dcd_filename
    last_fwd_frame = load_last_mdtraj_frame(
        dcd_filename, milestone.openmm.prmtop_filename)
  ...
  #{'CudaDeviceIndex':'0', 'CudaPrecision':'mixed', 'UseCpuPme':'false'}
  # add restraints
  #system, prmtop.topology, inpcrd.positions, 
  #  seekrcalc.min_equil.constrained)
  new_force = create_forces(seekrcalc, milestone, system) 
  system.addForce(new_force)
  if seekrcalc.umbrella_stage.barostat:
    barostat = MonteCarloBarostat(seekrcalc.umbrella_stage.barostat_pressure, 
                                  seekrcalc.master_temperature*kelvin, 
                                  seekrcalc.umbrella_stage.barostat_freq)
  ...
  if verbose: print "Running energy minimization on milestone:",\
       milestone.index
  ...
  simulation.reporters.append(DCDReporter(
      umbrella_traj, seekrcalc.umbrella_stage.traj_freq))
  starttime = time.time()
  ...
  # old way: simulate steps directly
