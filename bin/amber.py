"""
Merge ligand and receptor files to write parameter/topology/coordinate 
files for Amber simulations.

Created on May 10, 2018

@author: lvotapka, ideveci
"""
...
def amber_building(seekrcalc, milestone, amber_settings):
  ...
  Input:
   ...
   - amber_settings: the AmberSettings() object that defines settings 
       for the tleap calculation.
  Output:
   - None
  """
  i = milestone.index
...
def create_restraints(system, topology, constrained_list):
  ...
  Input:
   ...
   - constrained_list: a list of integers representing the indices of 
       the atoms to constrain.
  Output:
   - None
  """
...
def create_simulation(seekrcalc, milestone):
  ...
  if not milestone.openmm.prmtop_filename \
     or not milestone.openmm.inpcrd_filename:
    print "Amber parameter/topology/coordinate files not found for \
          milestone: %d. Skipping." % milestone.index
    return
  ...
  #, constraints=HBonds) # This is fine because 
  # h-bonds are always constrained in water!
  system = prmtop.createSystem(nonbondedMethod=PME, 
                               nonbondedCutoff=1*nanometer)
  #LangevinIntegrator(300*kelvin, 1/picosecond, 0.002*picoseconds)                             
  integrator = deepcopy(seekrcalc.min_equil.temp_equil_integrator)
  #Platform.getPlatformByName('CUDA')
  platform = seekrcalc.openmm.platform
  #{'CudaDeviceIndex':'0', 'CudaPrecision':'double'}
  properties = seekrcalc.openmm.properties
  
  # add restraints
  # TODO: clean this up
  create_restraints(system, prmtop.topology, seekrcalc.min_equil.constrained) 
  barostat = MonteCarloBarostat(1.0*bar, 
                                seekrcalc.master_temperature*kelvin, 25)
...
def save_restart(seekrcalc, milestone, pdb_save_filename=None):
  ...
  Input:
   - seekrcalc: The SeekrCalculation object that contains 
       all the settings for the SEEKR calculation.
   - milestone: the Milestone() object to prepare the AMBER system for
  Output:
   - None
  """
  if not milestone.openmm.prmtop_filename \
    ...
    print "Amber parameter/topology/coordinate files not found for \
          milestone: %d. Skipping." % milestone.index
    return
  state = milestone.openmm.simulation.context.getState(
      getPositions = True, enforcePeriodicBox = True)
  positions = state.getPositions()
  amber_parm = parmed.amber.AmberParm(milestone.openmm.prmtop_filename, 
