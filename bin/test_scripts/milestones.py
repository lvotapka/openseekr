...
class GeneratePlanarMilestoneAnchors:
	
	def __init__(self, lengths, com, max = 0):
	...
	def next(self):
		if self.n < self.max:
			self.com[2] = self.com[2] - self.lengths[self.n]
			anchor = []
			anchor.extend(self.com)	
			self.n +=1
		else:
            		raise StopIteration
		return anchor
	...
	def GenerateAnchors(self):
		PlanarAnchors = []
		for i in GeneratePlanarMilestoneAnchors(self.lengths, self.com, self.max):
			PlanarAnchors.append(i)
		return PlanarAnchors
