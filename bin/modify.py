...
assert (
    len(sys.argv) >= 3
), "At least two arguments required: 'command', and \
    'directory', and, possibly {arguments...}."
...
assert command in [
    ...
    Allowed commands: 'add', 'modify', 'delete', and 'report'."
if command == "add":
    assert (
        len(sys.argv) == 4
    ), "Only one extra argument allowed for 'add' \
      command: radius of adding milestone."
    args["radius"] = float(sys.argv[3])
elif command == "delete":
    assert (
        len(sys.argv) == 4
    ), "Only one extra argument allowed for 'delete' \
      command: index of milestone to delete."
    args["index"] = int(sys.argv[3])
elif command == "modify":
    # pass # more complicated situation...
    assert (
        len(sys.argv) == 5
    ), "Exactly two extra arguments allowed for 'modify'\
      command: index and radius of modified milestone."
    args["index"] = int(sys.argv[3])
    args["radius"] = float(sys.argv[4])
# print a concise report about all of the milestones in this system
elif command == "report":
...
assert os.path.exists(
    picklename
), "SEEKR pickle not found in provided \
...
if command == "add":
    ...
    assert (
        insert_index != None
    ), "The provided radius exists beyond the current \
      BD milestone. This is not currently supported. Move the existing BD \
      milestone, and add a new milestone below it."

    # create the new milestone
    new_milestone = seekr.Concentric_Spherical_Milestone(
        i, siteid, absolute, md=me.project.md, bd=me.project.bd
    )
    # TODO: may need to be modified in the future
    ...
    new_milestone.fullname = "%d_%d_%d_%.1f_%.1f_%.1f" % (
        insert_index + (spheres_in_site * int(siteid)),
        insert_index,
        siteid,
        anchor[0],
        anchor[1],
    ...
    # all upper milestones need modifying
    for i in range(insert_index, len(me.milestones)):
        ...
        id1, id2, mysiteid, anchor0, anchor1, anchor2 = me.milestones[i].fullname.split(
            "_"
        )
        new_fullname = "%d_%d_%s_%s_%s_%s" % (
            int(id1) + 1,
            int(id2) + 1,
            mysiteid,
            anchor0,
        ...
        if me.milestones[i].openmm.prmtop_filename:
            me.milestones[i].openmm.prmtop_filename = os.path.join(
                new_path, "md", "building", "holo.parm7"
            )
        if me.milestones[i].openmm.inpcrd_filename:
            me.milestones[i].openmm.inpcrd_filename = os.path.join(
                new_path, "md", "building", "holo.rst7"
            )
    ...
    anchor_filetree.make_tree(me.project.rootdir)
    # directory for MD
    md_dir = os.path.join(anchor_dir, "md")
    ...
    for i, milestone in enumerate(me.milestones):
        print "%d\t%f\t" % (
            milestone.index,
            milestone.radius,
        ), milestone.neighbors, "\t", milestone.directory
